...
class BusinessService {
    ...
    `;
        ...
        // Execute query
        const result = await database_1.pool.query(sqlQuery, params);
    ...
    async addToWatchlist(userId, organizationId, businessId, notes) {
    ...
    `, [(0, uuid_1.v4)(), userId, organizationId, businessId, notes || '']);
    }
    async removeFromWatchlist(userId, businessId) {
        await database_1.pool.query(`
      DELETE FROM business_watchlist 
      WHERE user_id = $1 AND business_listing_id = $2
    `, [userId, businessId]);
    }
    async getWatchlist(userId, organizationId) {
        const result = await database_1.pool.query(`
      SELECT 
        bl.*,
        bw.notes,
    ...
    async generateDueDiligenceReport(businessId, organizationId) {
    ...
    async calculateRiskAssessment(business) {
        ...
        const compositeRiskScore = Math.round((financialHealth * 0.4) +
            (legalRisk * 0.2) +
            (operationalRisk * 0.2) +
            (marketRisk * 0.2));
        // Risk grade
    ...
    assessFinancialHealth(business) {
    ...
    assessOperationalRisk(business) {
    ...
    generateRiskRecommendations(riskScore, grade) {
    ...
    async assessSBAQualification(business) {
    ...
    checkSBAIndustry(business) {
        ...
        if (business.industry) {
            return !ineligibleIndustries.some(industry => business.industry.toLowerCase().includes(industry));
        }
        return true;
    }
    calculateMaxSBALoan(business) {
        if (!business.financial_data.asking_price)
            return 0;
    ...
    async calculateROIProjection(business, investment) {
        const financial = business.financial_data;
        if (!financial.cash_flow || !financial.annual_revenue) {
            return {
                projected_roi: 0,
    ...
    async exportSearchResults(query, organizationId, format = 'csv') {
