...
interface SocialAuthButtonsProps {
  mode: 'login' | 'signup';
  onSuccess?: (provider: SocialProvider, isNewUser: boolean) => void;
  className?: string;
}
...
export const SocialAuthButtons: React.FC<SocialAuthButtonsProps> = ({
  ...
  const { signInWithSocial, isLoading } = useSocialAuth();

  const handleSocialAuth = async (provider: SocialProvider) => {
    const result = await signInWithSocial(provider, {
      organizationName: mode === 'signup' ? undefined : undefined
    });

    if (result.success && onSuccess) {
      onSuccess(provider, result.isNewUser || false);
    }
  };
