...
import { SubscriptionTier, SubscriptionLimits, AccessResult } from '../../types';
...
export class SubscriptionService {
  ...
  async checkAccess(
    ...
    const orgResult = await pool.query(queries.getOrganization, [organizationId]);
    if (orgResult.rows.length === 0) {
      return { allowed: false, reason: 'Organization not found' };
    }

    const tier = orgResult.rows[0].subscription_tier as SubscriptionTier;
  ...
  private async getCurrentUsage(organizationId: string, feature: keyof SubscriptionLimits): Promise<number> {
    const actionTypeMap = {
      searches: 'search',
      exports: 'export', 
      api_calls: 'api_call'
    ...
    const actionType = actionTypeMap[feature as keyof typeof actionTypeMap];
  ...
  async recordUsage(organizationId: string, actionType: string, quantity: number = 1) {
    await pool.query(queries.recordUsage, [
      uuidv4(),
      organizationId,
      actionType,
      quantity
  ...
  async upgradeSubscription(organizationId: string, newTier: SubscriptionTier) {
    ...
    switch (event.type) {
      ...
      case 'customer.subscription.deleted':
        ...
        if (subscription.status === 'canceled') {
          ...
          await pool.query(
            'UPDATE organizations SET subscription_tier = $1, status = $2 WHERE id = $3',
            ['starter', 'inactive', organizationId]
          );
        }
        break;
  ...
  private async getUsageSummary(organizationId: string) {
    const result = await pool.query(queries.getCurrentUsage, [organizationId]);
